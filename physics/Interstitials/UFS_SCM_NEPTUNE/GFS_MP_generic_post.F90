...
!> @{
      subroutine GFS_MP_generic_post_run(                                                                                 &
        ...
        frain, rainc, rain1, rann, xlat, xlon, gt0, gu0, gv0, gqtr0, prsl, prsi, phii, tsfc, ice, phil, htop, refl_10cm,  & 
        ...
        drain_cpl, dsnow_cpl, lsm, lsm_ruc, lsm_noahmp, raincprv, rainncprv, iceprv, snowprv, gq0,                        &
        graupelprv, draincprv, drainncprv, diceprv, dsnowprv, dgraupelprv, dtp,  dT_dt, dU_dt, dV_dt, dqtr_dt, dq_dt,     &
...
!
      ...
      real(kind=kind_phys), intent(in) :: dT_dt(:,:), dU_dt(:,:), dV_dt(:,:), dq_dt(:,:), dqtr_dt(:,:,:)
      real(kind=kind_phys), dimension(im,levs) :: save_t
      integer :: n

      save_t = gt0

      gt0 = gt0 + dT_dt * dtp * frain
      gu0 = gu0 + dU_dt * dtp * frain
      gv0 = gv0 + dV_dt * dtp * frain
      gq0 = gq0 + dq_dt * dtp * frain

      do n = 1, ntrac
        gqtr0(:,:,n) = gqtr0(:,:,n) + dqtr_dt(:,:,n) * dtp * frain
      end do
      ...
      if_radar: if(itime<=num_dfi_radar) then
        ...
        if(ldiag3d) then
           ...
           if(idtend_radar>0 .or. idtend_mp>0) then
              if(idtend_mp>0) then
                 dtend(:,1:2,idtend_mp) = dtend(:,1:2,idtend_mp) + (dT_dt(:,1:2) * dtp)*frain
              endif
              ...
              enddo
              if(idtend_mp>0) then
                 dtend(:,levs-1:levs,idtend_mp) = dtend(:,levs-1:levs,idtend_mp) + (dT_dt(:,levs-1:levs))*frain
              endif
           endif
        endif
      endif if_radar
           ...
           if_tracer_diagnostics: if (qdiag3d) then
              dtend_q: do itrac=1,ntrac
                 ...
                 if(idtend>=1) then
                    do k=1,levs
                       do i=1,im
                          dtend(i,k,idtend) = dtend(i,k,idtend) + (dqtr_dt(i,k,itrac) * dtp) * frain
                       enddo
                    enddo
                 endif
              enddo dtend_q
